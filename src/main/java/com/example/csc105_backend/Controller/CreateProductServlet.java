...
public class CreateProductServlet extends HttpServlet {
    ...
    private static final long serialVersionUID = -1119902997536249021L;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int result = createProduct.createProduct(product);
            CreateProductImage createProductImage = new CreateProductImage();
            if (result > 0) {
                boolean flag = true;
                for (Part part : parts) {
                    if (!part.getName().equalsIgnoreCase("data")) {
                        File file = new File(Constants.PATH_FILE+result+"\\");
                        if(!file.exists()) {
                            file.mkdir();
                        }
                        String fileName = getFileName(part);
                        fileNames.add(fileName);
                        String basePath =  Constants.PATH_FILE+result+"\\";
                ...
                if(!flag) {
                    ...
                    bean.setErrorMsg("Fail to Create Product Image");
                ...
                Error bean = new Error();
                bean.setErrorCode("400");
                bean.setErrorMsg("Fail to Create Product");
                json = new Gson().toJson(bean);
            }
