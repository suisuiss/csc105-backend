...
@WebServlet(name = "MyProductervlet", value = "/myProduct")
public class MyProductervlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter();){
            ...
            List<Product> result = getProductHistory.getProductHistory(Integer.valueOf(userId),"SELLER");
            ...
            if(null != result && !result.isEmpty()) {
                for(Product prod : result) {
                    ...
                    if(null != prodImg && !prodImg.isEmpty()) {
                        String[] urlSplit = request.getRequestURL().toString().split("myProduct");
